...
$$
    J = \pmb{U}_k^i \pmb{C}_{\pmb{XY}} {\pmb{{V}}_k^i}^T - \dfrac{1}{2} \lambda \left(\pmb{U}_k^i \pmb{C}_{\pmb{XX}} {\pmb{{U}}_k^i}^T - 1 \right) - \dfrac{1}{2} \theta \left(\pmb{V}_k^i \pmb{C}_{\pmb{YY}} {\pmb{{V}}_k^i}^T - 1 \right)
    \\
    \tag{1-1-5}
$$
对函数 $J$ 求偏导数并置零、化简：
...
朋友们我们今天来膜拜 (~~gank~~) 澳门大学的内卷发动机 *Chi Man Wong* 和 *Feng Wan* 老师团队。之所以对他“赞誉有加”，主要有三方面原因：
...
$$
    \begin{cases}
        \pmb{\mathcal{I}}_{M, N} = \left[\pmb{I}_N, ..., \pmb{I}_N \right]^T \in \mathbb{R}^{(MN) \times N}, \ \pmb{I}_N = diag(1, ..., 1) \in \mathbb{R}^{N \times N}\\
        \\
        ...
        \end{bmatrix}\\
        \\
        \bar{\pmb{X}} = \dfrac{1} {N_t} \pmb{\mathcal{I}}_{N_t, N_p}^T \cdot \left[\oplus_{i=1}^{N_t} \pmb{X}_i \right] \cdot \pmb{\mathcal{I}}_{N_t, N_c}
    \end{cases}
    \tag{3-2-2}
$$
...
言归正传，我们来看算法。由 3.1 节末尾分析可知，TRCA 类型的算法需要较多的训练数据以保证协方差矩阵估计的准确性，在原文中亦有相关描述：
...
$$
    ...
    \tag{3-3-1}
$$
这里我不想再去复述他们文章中对各种算法的具体匹配方式，仅在此对式 (3-3-1) 中的主要成分进行简单介绍：
