...
def trial_normalization(X: ndarray) -> ndarray:
    ...
    unit_var = np.std(X, axis=(1, 2), keepdims=True)
    return (X - trial_mean) / unit_var
...
def spatial_filtering(
    ...
    Args:
        ...
        X (ndarray): (Ne,Nc,Np) or (Ne*Nt,Nc,Np). Dataset.
        y (ndarray): If None, X is trial-averaged. If given, X is multi-trial dataset.

    Returns:
        wX (ndarray): (Ne,Nk,Np) or (Ne*Nt,Nk,Np).
    """
    # basic information
    n_components = w.shape[-2]  # Nk
    n_points = X.shape[-1]  # Np
...
def solve_coral(Cs: ndarray, Ct: ndarray) -> ndarray:
    """
    Solve CORAL problem: Q = min || Q^T Cs Q - Ct ||_F^2. <p>
    Q = Cs^(-1/2) @ Ct^(1/2).
