...
# %%
X = np.random.randn(40,10,9,500)
ew = trca_compute_e(X)

w = np.zeros((40,9))
for ne in range(40):
    w[ne,:] = trca_compute(X[ne,...])

print(np.max(abs(ew-w)))
...
S1 = np.zeros((40,9,9))
for ne in range(40):
    Xsum = np.sum(X[ne,...], axis=0)
    S1[ne,...] = Xsum @ Xsum.T
# %%
import scipy.io as io
data_path = r'E:\Document\SSVEP\Bench\Data\S1.mat'
data = io.loadmat(data_path)['data']

# %%
w1 = trca_compute_e(data[:,:5,:,500:1000])

w2 = np.zeros((40,64))
for ne in range(40):
    w2[ne,:] = trca_compute(data[ne,:5,:,500:1000])

see = w1 - w2
print(np.max(abs(see)))