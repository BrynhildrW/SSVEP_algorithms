...
def standardization(X: ndarray) -> ndarray:
    ...
    chan_var = np.std(X, axis=2, keepdims=True)
    return (X - trial_mean) / chan_var
...
def joint_trca_kernel_v2(
    ...
    """
    # basic information
    X_src_mean = utils.generate_mean(X=X_source, y=y_source)  # (Ne,Nc,Np)
    X_src_var = utils.generate_var(X=X_source, y=y_source)  # (Ne,Nc,Nc)
    X_tar_mean = utils.generate_mean(X=X_target, y=y_target)  # (Ne,Nc,Np)
    X_tar_var = utils.generate_var(X=X_target, y=y_target)  # (Ne,Nc,Nc)
    n_events = X_src_mean.shape[0]  # Ne

    # matrix Q: trial-averaged variance
    Q = (1 - theta) * X_tar_var + theta * X_src_var  # (Ne,Nc,Nc)

    # matrix S: covariance of trial-averaged data
    S = np.zeros_like(Q)  # (Ne,Nc,Nc)
    X_mix_mean = (1 - theta) * X_tar_mean + theta * X_src_mean
    for ne in range(n_events):
        S[ne] = X_mix_mean[ne] @ X_mix_mean[ne].T
...
def construct_prototype_filter(
    ...
    for nk in range(n_components):
        ...
        u[nk] = utils.solve_gep(A=Cuu, n_components=1, mode='Max')
    return u
...
def solve_noise_estimation(
    ...
    """
    Assume transformation matrix C satisfies the following conditions:
        (1) Source domain: N = X - A @ w @ X = X - A @ s
        (2) Target domain: X = A @ s + C.T @ N

    By solving problem:
        C = min || (A @ s + C.T @ N) (A @ s + C.T @ N).T - Var ||_F^2
          = min || C.T (N @ N.T) C - Var + A @ s (A @ s).T ||_F^2
...
class CLSDA(object):
    ...
    def margi_distr_alignment(self):
        ...
        self.P_mda, X_target_ea = utils.euclidean_alignment(self.X_target)
